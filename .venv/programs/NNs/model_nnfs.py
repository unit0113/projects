...
class Loss:
    ...
    def regularization_loss(self, Layer):
        ...
        if Layer.bias_regularizer_l2 > 0:
            regularization_loss += Layer.bias_regularizer_l2 * np.sum(Layer.biases * Layer.wbiases)
...
class Loss_CategoricalCrossentropy(Loss):
    ...
    def backward(self, y_pred, y_true):
        # Number of samples
        samples = len(y_pred)
        # Number of labels in every sample and we'll use the first sample to count them
        labels = len(y_pred[0])
...
class Optimizer_SGD:
    ...
    def update_params(self, Layer):
        if self.momentum:
            # Initialize momentum arrays if not previously done
    ...
    # Update iterations for next decay update
...
class Optimizer_Adagrad:
    ...
    def update_params(self, Layer):
        ...
        # Vanilla SGD update with normalization
