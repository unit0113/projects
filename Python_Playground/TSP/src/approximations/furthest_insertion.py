...
class FurthestInsertion(Approximation):
    ...
    def run(self) -> tuple[float, bool]:
        ...
        furthest_city = self._get_furthest()
        self._add_to_route(furthest_city)
    ...
    def _get_furthest(self) -> object:
        """ Get the city that is furthest to any city in the current route

        Returns:
            City: City in remaining cities that is furthest to a city in the current route
        """
        ...
        # Loop through remaining, find furthest
        ...
        return self.remaining_cities[furthest_idx]
