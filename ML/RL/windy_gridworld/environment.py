...
class GridWorld(object):
    def __init__(self, paint_maps=False):
        ...
        # Place walls on a bitmap
        self.walls = np.zeros(self.shape);
        for ob in self.obstacle_locs:
            self.walls[ob] = 3

        # Place absorbers on a grid for illustration
        self.absorbers = np.zeros(self.shape)
        for ab in self.absorbing_locs:
            self.absorbers[ab] = - 1

        # Place rewarders on a grid for illustration
    ...
    def draw_deterministic_policy(self, Policy):
        ...
        # The policy needs to be a np array of self.state_size values between 0 and 3 with 0 -> N, 1->E, 2->S, 3->W
    ...
    def build_grid_world(self):
        # Get the locations of all the valid states, the neighbours of each state (by state number), and the absorbing states (array of 0's with ones in the absorbing states)
    ...
    def plot_trace(self, trace, title=None, savefig=False, wait=False, waitforkey=False):
        # Trace should be a sequence of all states works only for linux Users
