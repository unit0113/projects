...
def SARSA(grid, num_episodes, start_loc=None, max_episode_len=100, gamma=1.0, epsilon=1.0, alpha=0.1):
    ...
    for _ in range(num_episodes):
        state_loc = get_state_loc(grid, start_loc)
        ...
        while (steps <= max_episode_len) & (not grid.absorbing[0, state_idx]):
            ...
            # Update Q function
            Q[state_idx, action_idx] += alpha * (reward + gamma * Q[state_prime_idx, action_prime_idx] - Q[state_idx, action_idx])
...
def Q_learning(grid, num_episodes, start_loc=None,  max_episode_len=100, gamma=1.0, epsilon=1.0, alpha=0.1):
    ...
    for _ in range(num_episodes):
        ...
        while (steps <= max_episode_len) & (not grid.absorbing[0, state_idx]):
            ...
            Q[state_idx, action_idx] += alpha * (reward + gamma * Q[state_prime_idx].max() - Q[state_idx, action_idx])
...
def epsilon_greedy_action(Q, epsilon, state_idx):
    if np.random.rand() < 1 - epsilon:
        ...
        return Q[state_idx].argmax()

    # p(a = a', a'!= a*|s) = epsilon / |A(s)|
    return np.random.choice(range(Q.shape[1]))


def get_state_loc(grid, start_loc):
    if start_loc == None:
    # Start at random location
        return random.choice(list(set(grid.locs) - set(grid.absorbing_locs)))
    return start_loc