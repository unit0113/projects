...
def worker(t, worker_model, counter, params):
    ...
    for i in range(params['epochs']):
        worker_opt.zero_grad()
        values, logprobs, rewards, G = run_episode(worker_env, worker_model) #B plays one episode
        actor_loss,critic_loss, eplen = update_params(worker_opt, values, logprobs, rewards, G) # Use collected data to run one update step
...
def run_episode(worker_env, worker_model, N_steps=10):
    ...
    while (j < N_steps and done == False):
        ...
        if done:
            reward = -10
            worker_env.reset()
        else: # Sets return to last state value
            reward = 1.0
            G = value.detach()
